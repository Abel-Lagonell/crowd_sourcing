...
class distance:
    ...
    def cost(self, vehicle, path):
        ...
        #//for t in path:
        distance += spd.pdist([currentPosition, self.tP[int(path)]])  # Check if t is target or target index!!!
        currentPosition = self.tP[int(path)]
...
class nashAssigner:
    ...
    def __init__(self, N, M, R, tau, dist=None, arrivalTime={}, decisionTime=0):  # initialize
        ...
        self.targets = np.zeros((self.N, 1), dtype='int')  # initalize by assigning random targets
        #//self.collection = list(permutations(range(0, M), 2))
        self.collection = list(range(0,M))
        ...
        for i in range(self.N):
            ...
            at = np.zeros(1)
            at = self.decisionTime + self.dist.travelTime(src=i, dest=index, start=True, finish=False)
            #//if index[0] != index[1]:
                #//at[1] = self.dist.travelTime(src=index[0], dest=index[1], start=False, finish=False) + at[0]
                #//self.arrivalTime[index[1]].append((i, at[1]))
            self.arrivalTime[index].append((i, at[0]))
    ...
    def getUtility(self, si, path):  # returns utility of vehicle si to go to target t (dependent on state of other vehicles)
        reward = self.getReward(si, path=path)
    ...
    def getReward(self, si, path):  # returns reward given to vehicle si to go to target t (dependent on state of other vehicles)
        reward2 = 0
        at = np.zeros(1)
        at = self.decisionTime + self.dist.travelTime(src=si, dest=path, start=True, finish=False)  # calculate arrival time of si at t
        ...
        reward1 = reward_calculations(self.R, self.tau, self.arrivalTime, path, at[0])
        #//if path[0] != path[1]:
            #//reward2 = reward_calculations(self.R, self.tau, self.arrivalTime, path[1], at[1])
    ...
    def pair(self, si, path):  # pair vehicle si with target t
        ...
        at[0] = self.decisionTime + self.dist.travelTime(src=si, dest=path, start=True, finish=False)
        self.arrivalTime[path].append((si, at[0]))
        self.arrivalTime[path].sort(key=lambda x: x[1], reverse=True)
    ...
    def successor(self, si, path):  # get the vehicle arriving after si at target t
        ...
        #//at[1] = self.dist.travelTime(src=path[0], dest=path[1], start=False, finish=False) + at[0]
        try:
            succ1 = next(x[0] for x in reversed(self.arrivalTime[path]) if x[1] > at[0])
    ...
    def getAssignments(self):
        ...
        for s in range(self.N):
            self.utilities[s] = self.getUtility(s, self.targets[s][0])
    ...
    def getVehicleReward(self, n):
        ...
        return self.getReward(n, self.targets[n][0])
...
def getVUtility(nA):
    ...
    for t in range(nA.N):
        if nA.targets[t][0] == nA.targets[t][0] and nA.targets[t][0] >= 0:
            value = nA.getUtility(t, nA.targets[t][0])
    ...
    for perm in collection:
        ...
        for i in range(N):
            ...
            for path in x:
                ...
                # calculate reward according to previous arrival time at t
                reward1 = reward_calculations(R, tau, arrivalTime, path[0], at[0])
                if path[0] != path[1]:
                    reward2 = reward_calculations(R, tau, arrivalTime,path[1], at[1])
...
def reward_calculations(R, tau, arrivalTime, path, at):
    try:
        pat = next(x[1] for x in arrivalTime[path] if x[1] < at)
        return R * (1 - np.e ** (-(at - pat) / tau))
    except StopIteration:
        return R
...
def largeVehicleRegimeTesting(i:int):
    n = 2
    nMax = 41
    Ns = range(n, nMax+1)
    M = 4
    ...
    with open('singleTestRangeN2-40_M4_Oct31-'+str(i)+'.csv', 'w') as out:
        for i, N in enumerate(Ns):
            # print(i, " ", N)
            out.write('%d,%d,%f,%f,%f,%f,%f,%f\n' % (N, nOfIterations[i], targetUtilities[i], 
                                                     platformReward[i], platformUtilperReward[i], 
                                                     maxUtility[i], minUtility[i], 
                                                     averageVehicleUtility[i]))
...
def printLargeVehicleRegimeTestingData():
    with open('Data10/doubleTestRangeAveraged.csv','r') as inp: #! File not here
        lines = inp.read().splitlines()

    with open('singleTestRangeAveraged.csv','r') as inp:
...
# Setup for Single
    ...
    plt.figure()
    plt.plot(Ns, platformUperR, label ='Double')
    plt.plot(Ns2,platformUperR2, label = 'Single')
    plt.axis([np.amin(Ns+Ns2)-1, np.amax(Ns+Ns2)+1, np.amin(platformUperR+platformUperR2), np.amax(platformUperR+platformUperR2)])
    plt.xlabel('Number Of Vehicles')
    plt.ylabel('Platform Utility per Reward')
    plt.title('Platform Utility per Reward as N increases M=4')
    plt.legend()

    plt.figure()
    plt.plot(Ns, platformReward, label ='Double')
    plt.plot(Ns2, platformReward2, label = 'Single')
    plt.axis([np.amin(Ns+Ns2)-1, np.amax(Ns+Ns2)+1, np.amin(platformReward), np.amax(platformReward+platformReward2)+3])
    plt.xlabel('Number of Vehicles')
    plt.ylabel('Total Dollars Given')
    plt.title('Platform Reward given out as N increases (M=4)')
    plt.legend()

    plt.figure()
    plt.plot(Ns, targetUtilities, label ='Double')
    plt.plot(Ns2, targetUtilities2, label ='Single')
    plt.axis([np.amin(Ns+Ns2)-1, np.amax(Ns)+1, np.amin(targetUtilities), np.amax(targetUtilities)+.2])
    plt.xlabel('Number Of Vehicles')
    plt.ylabel('Platform Utility')
    plt.title('Platform Utility as N increases (M=4)')
    plt.legend()

    plt.figure()
    plt.plot(Ns, vUtility, label = 'Double')
    plt.plot(Ns2, vUtility2, label = 'SingLe')
    plt.axis([np.amin(Ns)-1, np.amax(Ns)+1, np.amin(vUtility+vUtility2), np.amax(vUtility+vUtility2)])
    plt.xlabel("Number of Vehicles")
    plt.ylabel("Average Vehicle Utility")
    plt.title("Vehicle Utility as N increases (M=4)")
    plt.legend()

    plt.figure()
    plt.plot(Ns, maxUtility, color='blue', label = 'Double')
    plt.plot(Ns, minUtility, color='blue')
    plt.plot(Ns2, maxUtility2, color='orange', label = 'Single')
    plt.plot(Ns2, minUtility2, color='orange')
    plt.axis([np.amin(Ns)-1, np.amax(Ns)+1, np.amin(minUtility), np.amax(maxUtility)+.25])
    plt.xlabel('Number of Vehicles')
...
#THE GRAPHS YOU ARE LOOKING FOR ARE HERE